...
**判定には、壊れた例と「正しい書き方の例」を両方置く。** 文書の種類ごとに `expect_ok "…" check_xxx` を、Note の判定ごとに `expect_rule <rule>` と `expect_no_rule <rule>` を置く。**どちらも `validate.py` が数え上げており、欠けていれば落ちる。** 壊れた例だけを増やすと、判定を締めたときに正しい文書を巻き込んだことに気づけない（v1.5.2 と v1.5.3 で連続して誤検知を出した）。

**正しい例は、規約文書の例文をそのまま写さない。** 同じ文字列を置くと、判定を書いた本人の想定内しかテストされない。実際に壊れたのは想定外の書き方（`取得済み` という言い回し、コメント行の `ref:`）だった。
