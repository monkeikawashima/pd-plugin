...
├── skills/ux-*/ ほか22種            UI/UX スキル。**5層すべてに担当がある**（v1.2.0）
│                                  戦略 ux-research-planner（探索）/ strategy-insight-writer /
│                                       user-persona-builder / competitive-analysis
...
│                                       ux-design-review（作った後）/ ux-heuristic-eval（声が無い時）
│                                  表層 design-token-keeper / a11y-contrast-guard / figma-sync
...
**鮮度はユーザーに聞かず、自分で判定する。** MCP のレスポンスに含まれる更新日時（Notion `last_edited_time` / Drive `modifiedTime` / Slack `ts`）を読む。聞くのは取得を試みて取れなかった場合だけ。

- メタデータと本文の「最終更新」表記が食い違う場合は**古い方を採る**（新しく見せる方に倒さない）
- 分析対象の期間と比較し、期間より後の更新なら「2. 最新の仕様書」、期間より前で止まっていれば「6. 古い資料」へ自動で振り分ける
- 振り分け結果を一覧で可視化する（黙って格下げしない）。取得できなかったものは「鮮度不明」として、**現在値の根拠には使わない**（論点の把握には使ってよい）
- ユーザーが「これが最新」と言えば優先するが、メタデータ上の更新日が対象期間より古ければ**その食い違いを伝えてから**使う
- 出典を控える — 題名・URL・種別・更新日。読み取った日を更新日にしない
