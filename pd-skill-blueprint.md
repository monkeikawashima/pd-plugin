...
このファイルは、**任意のプロジェクトで Product Discovery Skill（`/pd:pd`）を再構築するための指示書**です。
...
このファイル自体は Skill の一部ではありません。`.claude/skills/pd/` の**外**に置いてください。Skill ディレクトリ内に置くと supporting file として読み込まれ、Skill の責務が濁ります。
...
分析結果は plugin の**外**、利用プロジェクト側に置く。ここは `/pd:pd-init` が作る。
...
**root 直下に置く旧レイアウトも読めるようにする。** v1.4.0 より前は `products/` `analyses/` … が root 直下にあった。検証器は分析データの根（`BASE`）を「`pd/` 配下にデータがあればそこ、無く root 直下にあれば root」と決める。**台帳のキーはこの根からの相対パスにする** — `ROOT` 相対にすると、`pd/` へ移した瞬間に全ファイルが「台帳にあるが存在しない」で落ちる。
...
`analyses/` `voices/` `simulations/` を `.claude/skills/pd/` の中に置いてはならない。supporting file として読み込まれ、Skill が過去の分析結果や全発話を毎回抱えることになる。
...
`/pd:pd` で人間が起動でき、Claude 自身も description から利用判断できる状態にする。
...
3. **使う場面 / 使わない場面**
4. **実行フロー13段**（下記）
5. **Step 1 対象プロダクトの特定** — `/pd:pd product-a` → `products/product-a.md` を参照。無ければ `_template.md` を Schema として実行時に構築。情報源の統合と信頼順序
...
| プロジェクト側の置き場所を root 直下に散らすか集約するか | **`pd/` 1つに集約**（v1.4.0）。例外は `CLAUDE.md` と `.github/workflows/validate.yml` の2つだけ | `products/` `analyses/` は汎用的な名前で、root 直下にあると pd のものか判別できない。**やめるときに何を消せばいいか分からない**状態は、導入の心理的コストを上げる。集約すると `/pd:pd-uninstall` の対象一覧も自明になる |
...
1. `.claude/skills/pd/framework/` と `.claude/skills/pd/products/` を作成
2. `SKILL.md` を作成
...
**`framework/` へのプロダクト固有情報の混入**

```bash
grep -rniE "固有名詞|固有KPI名" .claude/skills/pd/framework/
```
...
```bash
grep -rnE '^[A-Za-z][A-Za-z0-9 ,.:;()/&-]{30,}$' .claude/skills/pd/
```
...
for k in Controllability Interpretation Continue "Improvement Lever" "Leading Indicator" Voice "検出力" "見直し条件"; do
  printf '%-20s : ' "$k"; grep -rl "$k" .claude/skills/pd/ | xargs -n1 basename | tr '\n' ' '; echo
done
```
...
- [ ] `/pd:pd-uninstall` が、引数なしでは何も削除せずに対象を一覧できる
...
- `.claude/skills/pd/` を変更したら `pd-skill-blueprint.md` の該当セクションも同じ作業内で更新する（ユーザー確認は不要）
...
- 含めるもの: 何ができるか / 使い方（`/pd:pd` の打ち方と**聞き方のコツ**）/ **新しいプロダクトの追加手順**（§18 を素人向けに書き直したもの）/ **元データの用意の仕方**（3種類・4つの渡し方・置かないもの・匿名化・データが無い場合）/ フォルダの意味と「書き換えていいか」/ ラベルの考え方 / **分析結果の読み方**（Driver・Root Cause 等の用語を平易な言い換えで対応表に）/ 実際の分析例1つ / チームでの約束 / **やめ方**（プロジェクトからの片付けと plugin のアンインストール、その順序）/ FAQ
...
| 形式の混在 | `/pd:pd 名前` と `/pd:pd 〜を分析したい` が別々に登場し、正解が分からない | 「書き方は自由」と1回で示し、例を並べる |
| 内部説明の混入 | 内部のファイルパスや処理順を本文に書く | 見出しに「（知らなくても使えます）」と付ける |

**README とルール（`CLAUDE.md`）の同期を運用ルールにする。** Skill の変更で利用者の操作・置き場所・守るべきルール・用語が変わったら、README も同じ作業内で更新する。対応表を `CLAUDE.md` に置き、hook の文面にも README を含める（`.claude/skills/pd/` 配下の変更を検知した時点で両方を促す）。
...
| 層 | 仕組み | 何を守るか | git 必要 |
|---|---|---|---|
| セッション開始 | `SessionStart` hook | 残存違反の通知。git があれば hook も自動導入・自己修復 | 不要 |
...
### `commands/pd-uninstall.md` — 片付けられる状態を保つ
...
`CLAUDE.md` は読み飛ばされうるので、機械的な検知で補強する。`Write|Edit` の対象が `.claude/skills/pd/` 配下だった場合に、blueprint の同期を促す文脈を注入する。
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_input.file_path // .tool_response.filePath // empty' | { read -r f; case \"$f\" in */.claude/skills/pd/*) printf '%s' '{\"hookSpecificOutput\":{\"hookEventName\":\"PostToolUse\",\"additionalContext\":\"（blueprint 同期を促す文面）\"}}';; esac; }",
