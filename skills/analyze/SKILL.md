...
### 外部ソース（URL / MCP 経由）を読んだ場合

Notion・Google Drive・Slack などを MCP 経由で読める場合、URL を渡されたらその場で読んで情報源に加える。ただし**その場限りで消える情報**なので、扱いを固定する。

1. **出典を控える** — 何を読んだか（題名・URL・種別）と、**そこに書かれている更新日**。読み取った日を更新日として扱わない
2. **鮮度が不明なものを上位に置かない** — 更新日が確認できない外部ページは、上の信頼順序の「6. 古い資料」として扱う。ユーザーが「これが最新」と言った場合のみ「1」に上げる
3. **リンク先の記述をそのまま Fact にしない** — 外部ページの数字は「その資料にそう書いてある」ことが Fact であり、現在値である根拠ではない。実測と食い違う場合は実測を採る
...
### 外部ソース由来の更新は、黙って書き込まない

会話・実測・リポジトリから得た変化はそのまま反映してよい。**外部ソース（URL / MCP 経由）から読んだ値で Context を書き換える場合だけ、先に差分を出して選ばせる。** ユーザーが「反映して」と言うのを待たない。

```
このリンクから Context に反映できる変更が 3 件あります。

  1. 月間利用率           32% → 41%      出典: 〇〇 要件定義（更新日 2026-06-30）
  2. 主要タスク時間        未取得 → 4.2分  出典: 同上
  3. 保留中の見直し（追加）  新プラン導入の可否   出典: 同上

  [1] すべて反映する  [2] 選んで反映する  [3] 反映しない（今回の分析だけで使う）
```

規則:

- **上書きする値と、追記する値を分けて示す。** 上書きは現在値を失うため、変更前の値を必ず併記する
- **反映した値には出典を残す** — どの資料の、いつ時点の値か。書けない場合は反映しない
- **既存の実測値を、更新日が確認できない外部ページの値で上書きしない。** 提示はしてよいが、既定では選ばれた状態にしない
- **`Past Experiments` と `Decisions` は追記のみ。** 外部ソースを根拠に過去の決定や棄却結果を書き換えない（覆すなら新しい Decision として積む）
- 反映しないと選ばれた項目は、その分析の Note には残る。**Context に載らないだけで、調べ直しにはならない**
