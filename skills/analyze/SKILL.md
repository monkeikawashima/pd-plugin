...
**更新日はユーザーに聞かない。自分で取る。** MCP のレスポンスにはたいてい更新日時が含まれる（Notion `last_edited_time` / Drive `modifiedTime` / Slack `ts` など）。これを読み取って鮮度を判定する。「いつのものですか」と聞くのは、**取得を試みて取れなかった場合だけ**。

判定の順序:

1. **メタデータの更新日時を読む。** 本文に「最終更新: ...」等の記載があり、メタデータと食い違う場合は**古い方を採る**（新しく見せる方に倒さない）
2. **分析対象の期間と比べる。** 対象期間より後に更新されていれば「2. 最新の Product / PRD / 仕様書」、対象期間より前で止まっていれば「6. 古い資料」として扱う
3. **判定結果を黙って使わない。** どの資料をどちらに置いたかを、分析の中で見えるようにする

```
参照した外部ソース
  ✓ 〇〇 要件定義        更新 2026-07-28  → 最新として扱う
  ⚠ 〇〇 キックオフ議事録  更新 2025-11-02  → 古い資料として扱う（対象期間より前）
  ? 〇〇 共有メモ         更新日 取得できず  → 鮮度不明。現在値の根拠には使わない
```

ユーザーが「これが最新」と明示した場合はそれを優先する（信頼順序の「1」）。ただし**メタデータ上の更新日が対象期間より古い場合は、その食い違いを伝えてから使う。**

その他の規則:

- **出典を控える** — 題名・URL・種別・更新日。読み取った日を更新日として扱わない
- **鮮度不明のものを現在値の根拠にしない。** 論点の把握には使ってよいが、KPI の現在値としては採らない
- **リンク先の記述をそのまま Fact にしない** — 外部ページの数字は「その資料にそう書いてある」ことが Fact であり、現在値である根拠ではない。実測と食い違う場合は実測を採る
...
- `uiux/templates/` — 画面仕様 / ユーザーストーリー / UXDR / 検証計画 / ペルソナ / ジャーニー / デザイントークン / 探索計画 / ナビゲーション / デザインシステム / モック台帳
