...
3.7 **デザインシステムを決める** — UI/UX の作成とモック作成が**毎回同じものを参照する**ようにする

`/pd:design-system` の手順をそのまま実行し、`pd/specs/05-surface/design-system.md` を作る。

**先に検出してから聞く。** `package.json` の依存・`components.json`・`tailwind.config.*`・`.storybook/` を見て候補を出し、`AskUserQuestion` で選ばせる。ゼロから聞くと答えが揺れる。

**「無し（後で決める）」も選択肢に必ず入れ、選ばれたらそう記録する。** 導入直後にデザインシステムが決まっていないプロジェクトは普通にある。**未記入と「無いと決めた」を区別できないと、UI の作業のたびに同じ確認が繰り返される。**
...
- UI・モックを作る前に `pd/specs/05-surface/design-system.md` を読む。デザインシステムを変えるときは `/pd:design-system`
- モックを実装に落とす前に、そのモックの台帳（`pd/specs/05-surface/mocks/`）を作る（`/pd:mock-ledger`）。抽出は `mock_capture.py` が行い、手で数えない
- 実装は台帳のとおりに写す。文言は一字一句そのまま、色はトークン名、要素は全部。落とすなら台帳を `見送り: UXDR-…` に書き換えてから落とす
